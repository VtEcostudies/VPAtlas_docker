-- =============================================================================
-- 027_publish_constraints_in_column_comments.sql
-- =============================================================================
-- Rebuilds both canonical publication views so each column comment carries its
-- constraints alongside the prose description.
--
-- WHY
--   pg_featureserv publishes a column comment verbatim as that field's
--   description in the OGC collection, and publishes nothing else about the
--   field -- no length, no allowed values, no format. A client reading the OGC
--   route therefore has no way to size a string field and will infer one.
--   Inferred sizes are exactly how the published ArcGIS layers ended up with
--   String(4000) fields, which a VCGI schema review flagged as needless layer
--   storage and credit spend.
--
--   Appending the constraint to the prose is inelegant, but it is the only
--   channel that reaches EVERY OGC client, including a person reading the
--   collection page in a browser. The structured form is served separately at
--   /ogc/collections/{id}/schema, which nginx routes to the Node API because
--   pg_featureserv implements no such resource.
--
--   Example of the result:
--     'Dominant pool substrate. [max 20 characters; allowed values: Leaf
--      litter, Mud, Sand/Gravel, Bedrock, Other]'
--
-- The authored description text is unchanged; the constraint is appended at
-- generation time by _schema/build_views.js. Regenerate rather than editing the
-- statements below.
--
-- NOTE FOR OPERATORS
...
COMMENT ON COLUMN ogc.pool_visits."visitPoolTrees" IS
  'Percent of the pool shaded or occupied by trees, 0-100. [whole number]';
COMMENT ON COLUMN ogc.pool_visits."visitPoolShrubs" IS
  'Percent of the pool occupied by shrubs, 0-100. [whole number]';
COMMENT ON COLUMN ogc.pool_visits."visitPoolEmergents" IS
  'Percent of the pool occupied by emergent vegetation, 0-100. [whole number]';
COMMENT ON COLUMN ogc.pool_visits."visitPoolFloatingVeg" IS
  'Percent of the pool occupied by floating vegetation, 0-100. [whole number]';
...
COMMENT ON COLUMN ogc.pool_visits."visitFishSizeSmall" IS
  'Fish abundance recorded for the small size class. Sparsely recorded. [whole number]';
COMMENT ON COLUMN ogc.pool_visits."visitFishSizeMedium" IS
  'Fish abundance recorded for the medium size class. Sparsely recorded. [whole number]';
COMMENT ON COLUMN ogc.pool_visits."visitFishSizeLarge" IS
  'Fish abundance recorded for the large size class. Sparsely recorded. [whole number]';
...
COMMENT ON COLUMN ogc.pool_visits."visitSubmergedVeg" IS
  'Percent of the pool occupied by submerged vegetation, 0-100. [whole number]';
